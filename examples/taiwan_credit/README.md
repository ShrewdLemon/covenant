...
demo and 21 declared features. Every sample size in `covenants.yaml` was
set by measuring first; since v0.5 the one-hot logistic pipeline takes the
closed-form **linear-exact** attribution path, so the measured side is
exact rather than sampled and the full demo sequence runs in well under a
minute (timings below).
...
check reason-codes — taiwan-credit: PASS
  ...
  top-k jaccard    1.000  (threshold 0.60)
  background stability of measured side: 0.909
  measured via: linear-exact
  n denied evaluated: 300
# exit 0
```

The background size is the measured knob that matters here. Under the
sampled explainer this demo originally used, a 40-row background held only
a 0.744 top-4 jaccard across two seeded backgrounds — below Covenant's
0.8 floor — and the covenant's 120 rows were chosen by measuring 40/80/120
(0.744/0.819/0.916). The exact path keeps a residual dependence on the
background's *mean* (stability 0.909 above), so the measured 120 stands.
That is the intended workflow either way: run, observe, then set.
...
Since v0.5 the linear-exact path prices attribution in microseconds per
row here, so the sizes in `covenants.yaml` are generous rather than
squeezed — the comments in that file name the measurement behind each
one.
