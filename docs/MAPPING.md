...
| Declared feature set matches the features the model actually uses | Inventory-accuracy framing — SR 26-2 expects the inventory to give enterprise-level visibility of dependencies (as documented in the research guide); the declared-vs-used test itself is Covenant's construction, no section is claimed | Check 3 `covenant check features`: declared vs used features | shipped |
| Excluded variables stay excluded, and their obvious proxies are surfaced | The exclusion is the model owner's own documented claim; the association screen follows arXiv 2511.03807 (per the research guide). No regulatory pinpoint is claimed | Check 4 `covenant check exclusions`: measured influence of declared exclusions plus an association screen — proxies are **surfaced, not proven absent** | shipped |
